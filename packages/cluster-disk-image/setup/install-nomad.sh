...
function install_binaries {
  ...
  # Determine the appropriate architecture for the download URL
  local arch_suffix="amd64"
  if [[ "$arch" == "arm64" ]]; then
    arch_suffix="arm64"
  fi

  local readonly url="https://releases.hashicorp.com/nomad/${version}/nomad_${version}_linux_${arch_suffix}.zip"
  local readonly download_path="/tmp/nomad_${version}_linux_${arch_suffix}.zip"
  local readonly bin_dir="$path/bin"
  local readonly nomad_dest_path="$bin_dir/nomad"

  log_info "Downloading Nomad $version for architecture: $arch_suffix from $url to $download_path"
