...
function print_usage {
  ...
  echo "  install-nomad --version 0.6.3 --arch arm64"
}
...
function install_binaries {
  ...
  local readonly download_path="/tmp/nomad_${version}_linux_${nomad_arch}.zip"
...
function install {
  ...
  install_binaries "$version" "$path" "$user" "$arch_param"
