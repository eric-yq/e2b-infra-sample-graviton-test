...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -a, --architecture ARCH Specify the architecture to use (x86_64 or arm64)"
...
# Set architecture in config file if specified
if [[ -n "$ARCHITECTURE" ]]; then
    # Check if architecture is valid
    if [[ "$ARCHITECTURE" != "x86_64" && "$ARCHITECTURE" != "arm64" ]]; then
        echo "Error: Invalid architecture. Must be 'x86_64' or 'arm64'"
        exit 1
    fi
    
    # Check if CFNARCHITECTURE already exists in the config file
    if grep -q "^CFNARCHITECTURE=" "$CONFIG_FILE"; then
        # Update existing value
        sed -i "s/^CFNARCHITECTURE=.*/CFNARCHITECTURE=$ARCHITECTURE/" "$CONFIG_FILE"
    else
        # Add new value
        echo "CFNARCHITECTURE=$ARCHITECTURE" >> "$CONFIG_FILE"
    fi
    
    echo "Set architecture to $ARCHITECTURE in $CONFIG_FILE"
fi
...
# Save the architecture to the config file
if grep -q "^CFNARCHITECTURE=" "$CONFIG_FILE"; then
    ARCH_VALUE=$(grep "^CFNARCHITECTURE=" "$CONFIG_FILE" | cut -d'=' -f2)
    echo "architecture=$ARCH_VALUE" >> "$CONFIG_FILE"
else
    echo "architecture=x86_64" >> "$CONFIG_FILE"
fi
...
# Set admin token (hardcoded value)
echo "admin_token=:<YNmi&S+_KnROf-m!}kVBlb}heEgL-a" >> "$CONFIG_FILE"
