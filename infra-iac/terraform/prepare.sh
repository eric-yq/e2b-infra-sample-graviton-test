...
if [ -z "$CFNARCHITECTURE" ]; then
    echo "CFNARCHITECTURE not defined in config file, defaulting to x86_64"
    CFNARCHITECTURE="x86_64"
fi
...
# Ensure architecture is set in the output files
sed -i "s|\${CFNARCHITECTURE}|$CFNARCHITECTURE|g" "$TFVARS_OUTPUT" 2>/dev/null
